...
| Breast arterial calcification on mammogram | Breast arterial calcification is a non-coronary vascular calcification marker; it is not a CAC equivalent and does not diagnose coronary plaque. | partial | `breast_arterial_calcification` | aliases + severity | context only | risk-context sentence and CAC clarification when appropriate | patient context item | P1 |
